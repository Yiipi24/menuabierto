...
import { modoDeServicio } from "./servicios";
...
export function pedidosDe(r) {
  ...
  return {
    ...
    entregas: opcionesDeEntrega(r),
  };
}

// Cómo se quiere el pedido. Es lo primero que el dueño pregunta por chat
// —"¿para aquí o para llevar?"— y por eso va en el mensaje. Las opciones no
// se inventan: salen de lo que la ficha ya dice en `lib/servicios.js` (el
// modo de servicio y el servicio a domicilio), así que un local que solo
// vende para llevar nunca ofrece "para comer aquí".
export const ENTREGAS = [
  { slug: "sitio", nombre: "Para comer aquí", linea: "Para comer en el lugar" },
  { slug: "llevar", nombre: "Para llevar", linea: "Para llevar" },
  {
    slug: "domicilio",
    nombre: "A domicilio",
    linea: "A domicilio (te paso la dirección por aquí)",
  },
];

const ENTREGA_POR_SLUG = new Map(ENTREGAS.map((e) => [e.slug, e]));

export function entregaDe(slug) {
  return ENTREGA_POR_SLUG.get(String(slug ?? "")) ?? null;
}

export function opcionesDeEntrega(r) {
  const modo = modoDeServicio(r?.service_mode)?.slug ?? "ambos";
  const domicilio = (Array.isArray(r?.amenities) ? r.amenities : []).some(
    (a) => String(a ?? "").trim() === "domicilio",
  );
  return ENTREGAS.filter((e) => {
    if (e.slug === "sitio") return modo !== "solo-llevar";
    if (e.slug === "llevar") return modo !== "solo-sitio";
    return domicilio;
  });
}
 ...
 */
export function mensajeDePedido({ nombre, url, lineas, moneda = "MXN", entrega = null }) {
  const platillos = (lineas ?? []).filter((l) => l?.cantidad > 0);
  if (!platillos.length) return mensajeDeContacto(nombre, url);
  const como = entregaDe(entrega);
