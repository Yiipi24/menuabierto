...
// Las dietas que schema.org sabe nombrar. `RestrictedDiet` es un vocabulario
// cerrado —no tiene término para "keto" ni para "sin azúcar"— así que solo se
// declaran las que tienen el suyo y las demás se quedan en la carta, que es
// donde el comensal las lee de todos modos. Traducir "sin gluten" a
// `GlutenFreeDiet` es lo que hace que el buscador pueda contestar "vegano cerca
// de mí" con este platillo. Los alérgenos no entran: `MenuItem` no tiene dónde
// declararlos, y meterlos en la descripción sería inventar el dato del dueño.
const DIETA_SCHEMA = {
...
function dietasDe(platillo) {
  const dietas = (platillo.etiquetas ?? [])
    .map((e) => DIETA_SCHEMA[e.slug])
    .filter(Boolean);
  return dietas.length ? dietas : null;
}
