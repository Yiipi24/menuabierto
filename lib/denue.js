...
/**
 * Las cadenas entran al DENUE con el número de sucursal pegado delante del
 * nombre: "38224 STARBUCKS REVOLUCION", "M00020 MC DONALDS GONZALITOS". Ese
 * número es del sistema interno de la cadena; nadie lo dice ni lo busca.
 *
 * Pide cuatro dígitos o más y que queden al menos dos palabras, para no
 * romper los nombres que de verdad empiezan con un número: "100 Montaditos"
 * y "1000 Sabores" se quedan enteros.
 */
export function sinNumeroDeTienda(nombre) {
  const texto = String(nombre ?? "").trim();
  const resto = texto.replace(/^\p{L}?\d{4,}[\s.:·-]+/u, "").trim();
  if (resto === texto) return texto;
  return resto.split(/\s+/).length >= 2 ? resto : texto;
}

// Unidades que el DENUE clasifica en 7225xx pero donde no come nadie: la
// bodega o el estacionamiento de otro negocio, el club de nutrición que vende
// batidos de multinivel, la cooperativa de una escuela. Sembrarlas llena el
// directorio de fichas que nadie va a visitar ni reclamar.
//
// El patrón de las unidades de apoyo va anclado al principio a propósito: "La
// Bodega" es un bar, "Bodega de Tacos El Cuate" es una bodega. Y el club de
// nutrición pide la palabra "club" o la marca: "Mi Nutricion Favorita" podría
// ser comida saludable de verdad, así que pasa.
const DESCARTES = [
  [/^(bodega|almacen|auxiliar|estacionamiento|oficina)/, "unidad de apoyo"],
  [/\bherbalife\b|\bhierbalife\b|\bhierbalie\b/, "club de nutrición"],
  [/\b(club|clud|clib)\b.*\bnutricion/, "club de nutrición"],
  [/\b(cooperativa|tienda) escolar\b/, "cooperativa escolar"],
];

/**
 * Por qué esta ficha no se siembra, o null si sí. Se prueba sobre el nombre
 * ya legible; el motivo se devuelve para poder contarlo en la corrida en vez
 * de descartar en silencio.
 */
export function motivoDeDescarte(nombre) {
  const texto = sinAcentos(nombre);
  if (!texto) return null;
  for (const [patron, motivo] of DESCARTES) {
...
// El nombre del local tal como va a salir en la ficha: el del
// establecimiento, o la razón social si aquel viene vacío, ya legible y sin el
// número de sucursal de la cadena.
function nombreDeFila(fila) {
  const nombre = nombreLegible(limpio(fila.nom_estab)) || nombreLegible(limpio(fila.raz_social));
  return sinNumeroDeTienda(nombre);
}

/**
 * Por qué se descartó esta fila, o null si no se descartó por el nombre. Sirve
 * para que la corrida diga cuántas bodegas y clubes de nutrición dejó fuera;
 * una fila que no es restaurante devuelve null porque nunca fue candidata.
 */
export function descarteDeFila(fila) {
  if (!esRestaurante(fila.codigo_act)) return null;
  return motivoDeDescarte(nombreDeFila(fila));
}

/**
 * Una fila del CSV del DENUE, convertida a lo que se inserta en `restaurants`
 * más la cocina que le toca. Devuelve null si no es un restaurante, si es una
 * unidad donde no come nadie o si no tiene lo mínimo (nombre y municipio).
 */
export function fichaDesdeFila(fila) {
  ...
  if (motivoDeDescarte(nombre)) return null;
