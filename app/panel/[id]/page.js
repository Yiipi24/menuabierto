...
export default async function Editar({ params }) {
  ...
  const { data: restaurante } = await supabase
    .from("restaurants")
    .select(
      "id, name, summary, description, city, neighborhood, street, state, postal_code, phone, website, price_level, status, plan, premium_until, highlights, social_links, payment_methods, amenities, parking_cost, closed_days",
    )
    .eq("id", id)
